...
| scope     | Where a variable or function can be used. | (No set syntax — concept-based) | show an ID to scope |  |
| class     | Blueprint for creating objects with specific structure and behavior. | `class Dog {}` | classroom of students |  |
| object     | A specific version of a class. | `Dog myDog = Dog();` | my phone |  |
| property     | A variable that belongs to a class/object. | `String name;` | my house |  |
| method     | A function that belongs to a class. | `void bark() {}` | completing assignments |  |
| constructor     | A special function used to set up a class when it’s created. | `Dog(this.name);` | like a water bottle |  |
| abstraction     | Hiding the inner workings of code so users only interact with what they need. | (Concept — not specific code) | typing on a keyboard |  |
| override     | Changing how a built-in or inherited function behaves. | `@override` | driving a car |  |
| void     | A function that does not return a value. | `void printMessage() {}` | outer space |  |

## Flutter Definitions

| Term | Definition and Description | Base Structure | Real Life Example | App Example |
|------|----------------------------|----------------|-------------------|-------------|
| Main()     | A function that runs when your app starts. It tells Flutter what app to show. | `void main() => runApp(MyApp());` | A business |  |
| Material App     | The widget that sets up your whole app’s look and navigation. | `MaterialApp(...)` | A social media app |  |
| Scaffold     | A widget that gives you the basic layout: background, navigation bar, floating button, etc. | `Scaffold(...)` | used in construction |  |
| Column     | A widget that holds and displays your content in a straight line from top to bottom. | `Column(...)` | A row in math |  |
| Row     | A widget that shows things side-by-side. | `Row(...)` | Sitting row |  |
| container     | A box that holds other widgets. You can add color, padding, borders, or size. | `Container(...)` | store food |  |
| text     | A widget to display text on the screen. | `Text('Hello')` | imessages |  |
|  image.network    | A widget to show an image using a link from the internet. | `Image.network('https://...')` | google images |  |
| elevatedbutton     | A clickable button that floats above content. You choose what happens when it's clicked. | `ElevatedButton(onPressed: ..., child: ...)` | buttons |  |
| onPressed     | The code that gets run when a button is tapped or something happens. | `onPressed: () => doSomething()` | save information |  |
